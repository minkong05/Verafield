...
The repo is a monorepo skeleton: `apps/` (two deployables), `backend/` (one API service), `packages/` (shared types). Only `backend/` and `packages/shared_types/` have real code today — `apps/field-collector` and `apps/mill-dashboard` have no chosen stack yet. Under `backend/services/` and `backend/db/`, only `gap_assessment` (roadmap feature 01) and `rules_engine` (feature 02) are implemented end-to-end; `verification_engine`, `evidence_pack`, `national_integration`, and `renewal` are still README-only. Check each folder's README before assuming an interface exists.
...
# Tests that touch the DB (test_household.py, test_gap_assessment.py, test_rules_engine.py, test_db_models.py)
